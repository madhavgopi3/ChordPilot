...
For the chord output display, we tried various options such as lists and sheet music display but finally decided on implementing the output on canvas, to improve support for all users. The canvas is drawn using a 2D array of strings. The cell width and height are calculated by dividing the total width and height by the number of columns and the number of cells in the longest column. The harmonizer object stores the indices of the chosen chords, so the corresponding cells could be colored differently. When the user clicks anywhere in the canvas, an function is triggered that find out which cell was clicked using the x and y coordinates of the canvas origin and the click event. The appropriate functions are then called to re-calculate the chord scores and redraw the canvas to update the output.
